...
public class EmployeeSpecialApplication implements Initializable {
    ...
    void sendApplication(MouseEvent event) throws IOException {
        ...
        if(leavePeriod.equals("Unbound") && leaveReason.equals("")){
            ...
            image_dialog.getIcons().add(new Image("/javafx_images/icons8_timetable_60px_1.png"));
            alert.showAndWait();
        }else{
            ...
            String exists;
            exists = "SELECT COUNT(1) FROM `tbl_leavedetails` WHERE leave_staff= '" + EmployeeLogin.loggedUser_id + "' AND " +
                    "leave_type='"+type+"' AND NOT leave_status='Accepted' ";
            ...
            try {
                ...
                ResultSet resultSet = statement1.executeQuery(exists);
                ...
                while (resultSet.next()) {
                    if (resultSet.getInt(1) == 1) {
                        ...
                        alert.setContentText("You already made an Application." +
                                "\nWait for Approval before applying for another.");
                        Stage image_dialog = (Stage) alert.getDialogPane().getScene().getWindow();
                        image_dialog.getIcons().add(new Image("/javafx_images/icons8_timetable_60px_1.png"));
                        alert.showAndWait();
                        ...
                        try{
                            ...
                            Stage image_dialog = (Stage) alert.getDialogPane().getScene().getWindow();
                            image_dialog.getIcons().add(new Image("/javafx_images/icons8_timetable_60px_1.png"));                            alert.setContentText("Leave Applied for Successfully.");
                            alert.showAndWait();
